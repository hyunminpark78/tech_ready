    ...
    <div>
      ...
      <div class="relative p-8 mb-8 bg-white border rounded-lg border-primary-200 dashboard-card" style="height: 520px">
        <template v-if="hasData">
          <!-- 민감도 설정 팝업 -->
          <FraudDetectionSensitivity v-show="isSense" :sense-set-val="senseVal" @sensePopup="sensePopup" />
          <FraudDetectionUserArmIntvl
            v-show="isIntvl"
            :user-arm-intvl="intvlVal"
            :curcy-cd="getCurcyCode()"
            @intvlPopup="intvlPopup"
          />
          <!--// 민감도 설정 팝업 -->
          <div>
            <div class="flex items-center justify-between">
              <h3 class="mb-1 font-bold">{{ $t('cost.costAnomalyDetection') }}</h3>

              <!-- 민감도 설정 버튼 -->
              <div class="flex items-center">
                <button class="flex items-center text-sm text-gray-500 mr__10 letter-spacing---5" @click="sensePopup">
                  <span class="mr-1">{{ $t('cost.sensitivitySettings') }}</span>
                ...
                <button class="flex items-center text-sm text-gray-500 ml__10 letter-spacing---5" @click="intvlPopup">
                  <span class="mr-1">{{ $t('cost.alarmSectionSettings') }}</span>
                  <img src="@/assets/images/ico-flip.svg" alt="." />
                </button>
              </div>
              <!--// 민감도 설정 버튼 -->
            ...
            <!-- 2022-02-22 : 추가 -->
            <div v-if="i18n().locale === 'ko'" class="info__txtbox mb__20">
              <p class="mb__5">
                {{ $t('cost.sensitivity') }} : <span>{{ getSenseMsg() }}</span>
              </p>
            ...
            <div v-if="i18n().locale === 'en'" class="info__txtbox mb__20">
              <p class="mb__5">
                {{ $t('cost.sensitivity') }} : <span>{{ getSenseMsg() }}</span>
              </p>
...
<script>
import FraudDetectionSensitivity from '@/pages/Analysis/FraudDetection/FraudDetectionSensitivity.vue';
import FraudDetectionUserArmIntvl from '@/pages/Analysis/FraudDetection/FraudDetectionUserArmIntvl.vue';
import FraudDetectionDetail from '@/pages/Analysis/FraudDetection/FraudDetectionDetail.vue';
...
export default {
  ...
  mounted() {
    ...
    if (this.$route.params.ctrtId && this.$route.params.detail !== 'Y') {
  ...
  methods: {
    ...
    getSenseMsg() {
      ...
      return val === 'H' ? this.$t('cost.high') : val === 'L' ? this.$t('cost.low') : this.$t('cost.medium');
    },
    popDetail(data) {
      if (data) {

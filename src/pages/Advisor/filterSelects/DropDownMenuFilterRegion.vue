<template>
  <div v-click-outside="handleClickOutside" style="width: 100%; padding-left: 18px; position: relative">
    <button :class="selectClass" :disabled="pending || !hasData" @click="toggle">
      <template v-if="noItemChecked">
        <span class="text-primary-400">{{ $t('advisor.gpuSPot.gpuSpotFilter.choice') }}</span>
      </template>
      <template v-else>
        <span class="text-primary-400">{{ sortedData.map((e) => e.nm).toString() }}</span>
      </template>
    ...
    <div
      ...
      style="max-height: 385px; overflow-y: hidden; width: 100%"
    >
      <div style="overflow-y: scroll; max-height: 344px">
        <div
          class="relative bg-white rounded dashboard-card select-two"
...
import PerformanceAcntSelectCount from '@/pages/Rsrc/Performance/PerformanceAcntSelectCount.vue';

export default {
  components: { Select, SelectCountOption, PerformanceAcntSelectCount },
  mixins: [selectMixin],
  props: {
  ...
  data() {
    return {
      ...
      selectedCustCorpIds: [],
      closeList: this.data.map((item) => item.id),
    };
  },
  computed: {
    isOpened() {
    ...
    sortedData() {
      const parentObjects = this.checkedItems.filter((obj) => Array.isArray(obj.childKey));

      // If no objects with childKey are found, return the original array
      if (parentObjects.length === 0) {
        return this.checkedItems;
      }

      // Create a Set of all child ids
      const childIds = new Set(parentObjects.flatMap((parent) => parent.childKey.map((child) => child.id)));

      // Filter the array
      return this.checkedItems.filter((obj) => {
        // Keep if it's a parent object (has childKey)
        if (Array.isArray(obj.childKey)) return true;

        // Keep if it's not in the childIds Set
        if (!childIds.has(obj.id)) return true;

        // Exclude if it's a child object
        return false;
      });
    },
  },

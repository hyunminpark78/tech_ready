<template>
  <div v-click-outside="handleClickOutside">
    ...
    >
      <div style="overflow-y: scroll; max-height: 344px; width: 100%">
              ...
              <li v-for="ctgry in item[childkey]" :key="ctgry.id">
                ...
                <ul style="margin-left: 31px">
                  <PerformanceAcntSelectCount
                    v-for="grp in ctgry[secondChildkey].filter((x) => x.nm.toLowerCase().includes(keyword.trim()))"
                    :key="keyGetter(grp)"
                    type="check"
                    svc="svc"
...
export default {
  components: { PerformanceAcntSelectCount },
  mixins: [selectMixin],
  props: {
  ...
  methods: {
    ...
    cancelSvcGrpSelect() {
      if (this.previousSelectedCustCorpIds && this.previousSelectedCustCorpIds.length > 0) {
        ...
        // this.svcGrpSelectedCustCorpIds = [...this.previousSelectedCustCorpIds];
        /* this.$refs.svcCustCorpSel.handleItemClick(this.previousSelectedCustCorpIds[0]);*/
      }
      // this.checkedItems = [...this.previousCheckedItems];

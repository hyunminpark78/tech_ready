<template>
  <div id="detailTable" class="box-wrap">
    ...
    />
    <SchedulingRecommendGPUPop :visible="isPopupVisible" @close="popupClose" />
  </div>
</template>
<script>
...
import SchedulingRecommendGPUPop from './SchedulingRecommendGPUPop.vue';
...
export default {
  ...
  computed: {
    ...mapState('resourceScheduling', {
      setRecommendGPUDetailData: 'setRecommendGPUDetailData',
      filter: 'filter',
  ...
  watch: {
    ...
    setRecommendGPUDetailData: async function () {
      if (this.setRecommendGPUDetailData) {
        await this.fetchRecommendGPUDetailData(false);
  ...
  methods: {
    ...mapActions('resourceScheduling', ['fetchRecommendGPUDetailData', 'fetchSchedulingGPUChartData']),
    initPageObj() {
    ...
    async setData() {
      ...
      try {
        this.isLoading = true;
        const response = await resourceScheduleService.fetchSchedulingRecommendGPUDetail(payload);
        const dataList = response.data;
        this.rowData = dataList || [];

        // Summary Component에서도 활용
        this.fetchSchedulingGPUChartData(this.rowData);
      } catch (e) {
        if (!axios.isCancel(e)) {

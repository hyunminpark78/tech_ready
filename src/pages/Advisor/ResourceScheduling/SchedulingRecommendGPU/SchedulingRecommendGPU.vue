  ...
  <div class="contents-wrap">
    ...
    <div class="contents">
      ...
      <SectionNewHeader
        ...
        :title2="$t('advisor.schedulingRecommendGPU.title')"
        :title="$t('advisor.schedulingRecommendGPU.title')"
      />
      <!-- header -->
      <Section>
        <SectionMain style="display: flex; flex-direction: column; gap: 20px">
          <MainFilter @filterChanged="onFilterChanged" />
          <SchedulingEstimatedSavingsGPU />
...
import { MainFilter, DetailTable, ComparisonPopup, SchedulingEstimatedSavingsGPU } from './sections';
...
export default {
  ...
  methods: {
    ...mapActions('resourceScheduling', ['fetchRecommendGPUDetailData']),
    setAllItem() {
      if (i18n.locale === 'ko') {
        this.allItem = {
          id: 'checkAll',
    ...
    onFilterChanged() {
      this.fetchRecommendGPUDetailData(true);
    },
  },
};
</script>

<template>
  <div class="box-wrap pb24 list-wrap">
    ...
    <div class="list7-wrap">
      ...
      <div class="tit4-wrap" style="margin: 18px 0 0 0">
        {{ $t('dashboard.costPatternAnalytics') }}
        <button class="more single" @click="routeCostPattern(null)">{{ $t('common.button.viewDetails') }}</button>
      </div>

      <DataLoadingOverlay v-if="patternLoading" :height="211"></DataLoadingOverlay>
      <NoDataOverlay v-else-if="!existPatternData" :icon-type="'chart'" :height="225"></NoDataOverlay>
      <template v-if="existPatternData && !patternLoading">
        <CardMessage>
          <OcpTextInline>{{ $t('dashboard.patternGuideMessage1', { value: patternGuide.service }) }}</OcpTextInline>
          <OcpTextInline secondary>
            {{ $t('dashboard.patternGuideMessage2', { value: patternGuide.rate }) }}</OcpTextInline
          >
          <OcpTextInline>{{ $t('dashboard.patternGuideMessage3') }}</OcpTextInline>
        </CardMessage>
      </template>
      <div
        v-show="existPatternData && !patternLoading"
        id="patternChart"
        class="chart-wrap"
        style="height: 171px"
      ></div>

  ...
  <Card :wrapper-class="costClass">
    ...
    <template>
      <div class="flex items-center my-2">
        <div v-if="true" class="relative ml-6 bg-white dashboard-card w-52">계약별({{ ctrtLength }})</div>
        <div class="mr-auto">
...
export default {
  ...
  computed: {
    ...
    convertData() {
      ...
      topData = this.mergeDate(topData);
      ...
      if (this.selectCategory !== 'CTRT') {
        ...
        Object.entries(etcData).map(([d, data], index) => {
          ...
          t['grpId'] = 'ETC';
          t['grpNm'] = 'ETC';
          t['accumKrwUnblendCost'] =
            data[index] && data[index].accumKrwUnblendCost ? data[index].accumKrwUnblendCost : 0;
          etcList.push(t);
        });
        etcList = this.mergeDate([etcList]);
        etcList = etcList[0];
        var bfSum = 0;
        etcList.forEach((item, index) => {
          bfSum += item.accumKrwUnblendCost;
          item.sum = bfSum;
        });

        etcList.forEach((item, index) => {
          item.accumKrwUnblendCost = item.sum;
        });

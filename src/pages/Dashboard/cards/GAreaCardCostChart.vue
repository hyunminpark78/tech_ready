<template>
  <div :style="styleObject" ref="amChartArea" style="overflow: hidden!important;"></div>
</template>
...
export default {
  name: 'GAreaCardCostChart',
  componentName: 'GAreaCardCostChart',
  props: {
    data: {
      type: Array,
      default: () => [],
      ...
      switch (this.chartTheme) {
        ...
        case 'dash_abNormalDetect':
          ...
          if (i18n.locale === 'ko') {
            ...
            this.chartOption.series.customName = ['Actual', 'Predicted', 'Anomaly', 'Sensitivity'];
          }
          this.chartStyle = [
            {
              fill: false,
      ...
      if (this.chartOption.series.isCustomTooltip && this.chartTheme === 'dash_abNormalDetect') {
        ...
        series.adapter.add('tooltipHTML', (html, target) => {
          ...
          let strHtml =
            ...
            '</span> </br>';
          strHtml +=
            `AI ${this.$t('cost.predictedCost')} :   ` +
            "<span class='custom_detect_cont'>" +
            curcy +
            forcstPrice +
            '</span> </br>';
          strHtml +=
            `${this.$t('cost.actualCosts')} :   ` +
            "<span class='custom_detect_cont'>" +
            curcy +
            realPrice +
            '</span> </br>';
          strHtml +=
            `${this.$t('cost.rateOfChange')} :   ` + "<span class='custom_detect_cont'>" + chgPer + '%</span></div>';
          return strHtml;
        });
      }

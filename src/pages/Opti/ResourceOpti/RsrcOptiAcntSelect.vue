...
import RsrcAcntSelectCount from '@/pages/Opti/ResourceOpti/RsrcAcntSelectCount';
import { i18n } from '../../../../public/locales/i18n';
export default {
  components: { RsrcAcntSelectCount, Select, SelectCountOption },
  mixins: [selectMixin],
  props: {
  ...
  computed: {
    ...mapState('resourceOpti', { filter: 'filter', storeCategory: 'category', companyId: 'selectedCustCorpIds' }),
    isOpened() {
  ...
  methods: {
    ...
    //     text += ` ${this.$t('optimization.notConnected')}`;
    //   }
    //   return text;
    // },
    ...
    handleCheckAcntClick(contractId, acntId) {
      const matchingCustCorp = this.data.find((item) => item.id === contractId);
      const selectedCorpRegNo = matchingCustCorp.corpRegNo;
      const selectedCustCorp = this.custCorpList.find((item) => item.corpRegNo === selectedCorpRegNo);
      const matchingAcnt = matchingCustCorp.acntList.find((item) => item.id === acntId);
      this.handleCustCorpChange(selectedCustCorp);
      this.updateCheckedItem(matchingAcnt, true);
    },
  },
};
</script>

<template>
  <div class="flex items-center justify-center my-7 pagination">
    <button :style="disablePrev" @click="clickPage(1)">
      <img src="@/assets/images/btn-first.svg" alt="처음 페이지" />
    </button>
    <button :style="disablePrev" @click="onBtPrevious">
    ...
    <button :style="disableNext" @click="onBtNext">
      <img src="@/assets/images/btn-pre.svg" class="rotate-180-" alt="다음 페이지" />
    </button>
    <button :style="disableNext" @click="clickPage(totalPages)">
...
export default {
  ...
  methods: {
    clickPage(page) {
      this.$emit('clickPage', page - 1);
    },
    onBtPrevious() {
      this.$emit('onBtPrevious', this.currentPage);
    },
    onBtNext() {
      this.$emit('onBtnNext', this.currentPage);
    },
  },
};
</script>
